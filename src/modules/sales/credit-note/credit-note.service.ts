...
export class CreditNoteService {
    ...
    async create(businessId: number, memberId: number, data: CreateCreditNoteInterface) {
        ...
        // ------------------------------------------------------------------
        // PASO 1: OBTENER DATOS Y VALIDAR PERTENENCIA
        // ------------------------------------------------------------------
        const sale = await prisma.sale.findUnique({
            ...
            include: {
                items: {
                    include: {
                        // CRÍTICO: Aquí sabemos qué ingredientes/lotes exactos se usaron
                        lotAllocations: true, 
                        product: true 
                    }
                },
                creditNotes: {
                    include: { items: true } // Para saber qué se ha devuelto antes
                }
            }
        });

        if (!sale) throw new BusinessError('Venta no encontrada', 404);
        if (sale.status === SaleStatus.CANCELLED) throw new BusinessError('No se puede generar nota de crédito sobre una venta anulada', 400);
        ...
        for (const requestItem of data.items) {
            ...
            const originalSaleItem = sale.items.find(i => i.productId === requestItem.productId);
            ...
            const previouslyReturned = sale.creditNotes.reduce((acc, cn) => {
        ...
        // ------------------------------------------------------------------
        // PASO 3: TRANSACCIÓN ATÓMICA (ESCRITURA)
        // ------------------------------------------------------------------
        const result = await prisma.$transaction(async (tx) => {
            ...
            // =========================================================
            // PASO C: CÁLCULO FINANCIERO (CRÍTICO) 💰
            // =========================================================
            
            let amountAppliedToDebt = new Decimal(0);
            let amountToReturnCash = new Decimal(0);

            // 1. Logica de Distribución
            if (currentSale.remainingBalance.gt(0)) {
                // Si debe dinero, PRIORIZAMOS pagar la deuda.
                if (totalRefundAmount.gte(sale.remainingBalance)) {
                    // Caso A: La devolución CUBRE toda la deuda y sobra dinero.
                    // Ej: Debe $20, Devuelve $100.
                    amountAppliedToDebt = sale.remainingBalance;       // Bajamos $20
                    amountToReturnCash = totalRefundAmount.sub(sale.remainingBalance); // Sobran $80
                } else {
                    // Caso B: La devolución es MENOR a la deuda.
                    // Ej: Debe $100, Devuelve $20.
                    amountAppliedToDebt = totalRefundAmount;           // Bajamos $20
                    amountToReturnCash = new Decimal(0);               // No sobra nada
                }
            } else {
                // Caso C: Venta de Contado (No debe nada).
                // Ej: Debe $0, Devuelve $50.
                amountAppliedToDebt = new Decimal(0);
                amountToReturnCash = totalRefundAmount;                // Se devuelve todo ($50)
            }

            // 2. Ejecutar Actualizaciones en Base de Datos (Solo si aplica)
            if (amountAppliedToDebt.gt(0)) {
                // A. Reducir deuda de la Venta
                await tx.sale.update({
                    where: { id: sale.id },
                    data: { remainingBalance: { decrement: amountAppliedToDebt } }
                });

                // B. Reducir deuda Global del Cliente
            ...
            return {
                creditNote, // El registro creado
                financialAction: {
                    action: amountToReturnCash.gt(0) ? 'REFUND_CASH' : 'ADJUST_DEBT',
                    totalRefunded: totalRefundAmount,
                    
                    // Desglose
                    appliedToDebt: amountAppliedToDebt,
                    cashToReturn: amountToReturnCash,
                    
                    // Mensaje sugerido para el UI
                    message: amountToReturnCash.gt(0) 
                        ? `⚠️ ENTREGAR EFECTIVO: $${amountToReturnCash.toFixed(2)}` 
