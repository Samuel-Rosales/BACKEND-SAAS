...
export class CreditNoteService {
    ...
    async create(businessId: number, memberId: number, data: CreateCreditNoteInterface) {
        try {
            ...
            const result = await prisma.$transaction(async (tx) => {
                ...
                if (amountToReturnCash.gt(0)) {
                    ...
                    const exchangeRate = await tx.exchangeRate.findUnique({
                        where: { id: currentSale.exchangeRateId, isActive: true }
                    });

                    if (!exchangeRate) {
                        throw new BusinessError('No se encontró la tasa de cambio activa para procesar la devolución en efectivo.', 500);
                    }

                    // Validar que la tasa no sea cero para evitar divisiones infinitas
                    if (new Decimal(exchangeRate.rate).equals(0)) {
                    ...
                    for (const payment of incomingPayments) {
                        ...
                        if (method.currency === 'USD') {
                            ...
                            valueInUsd = paymentAmount.div(exchangeRate.rate);
                        }
                    ...
                    if (diff.gt(EPSILON)) {
                        throw new BusinessError(
                            ...
                            `Los pagos ingresados suman $${totalPaidInBaseCurrency.toFixed(2)} (al cambio de la tasa ${exchangeRate.rate} del día de la venta).`, 
                            400
                        );
                    }
                    ...
                    for (const payment of incomingPayments) {
                        ...
                        const rateIdToSave = method.currency !== 'USD' ? exchangeRate.id : exchangeRate.id; 
            ...
            return { status: 500, message: 'Error interno al crear la nota de crédito', data: null };
        }   
    }
